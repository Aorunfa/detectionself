...
class ComputeLoss:
    ...
    def bbox_decode(self, anchor_points, pred_dist):
        if self.use_dfl:
            ...
            pred_dist = pred_dist.view(b, a, 4, c // 4).softmax(3).matmul(self.proj.type(pred_dist.dtype)) # integration for 16 points , dist sum = 1
    ...
    def __call__(self, p, targets, img=None, epoch=0):
        ...
        anchor_points, stride_tensor = make_anchors(feats, self.stride, 0.5)                        # get anchor xyxy and downsample rate

        # targets
        targets = self.preprocess(targets, batch_size, scale_tensor=imgsz[[1, 0, 1, 0]])
        gt_labels, gt_bboxes = targets.split((1, 4), 2)  # cls, xyxy
        mask_gt = gt_bboxes.sum(2, keepdim=True).gt_(0)  # know padd and real true

        # pboxes
        pred_bboxes = self.bbox_decode(anchor_points, pred_distri)  # xyxy, (b, h*w, 4), bbox = anchor cxcy + anchor offset
        ...
        loss[1] = self.BCEcls(pred_scores, target_scores.to(dtype)).sum() / target_scores_sum # BCE avg
        loss[1] *= 0.25
        loss[1] += self.BCEcls(pred_scores2, target_scores2.to(dtype)).sum() / target_scores_sum2 # BCE avg
